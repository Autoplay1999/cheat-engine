...
begin
  ...
  try
    oldAddressFile:=TFileStream.Create(folder+'A.TMP', fmopenread or fmShareDenyNone);
    oldMemoryFile:=TFileStream.Create(folder+'M.TMP', fmopenread or fmShareDenyNone);
    ...
    begin
      ...
      NewAddressFile:=TFileStream.Create(folder+'A.FIRST', fmCreate);
      NewMemoryFile:=TFileStream.Create(folder+'M.FIRST', fmCreate);
    ...
    begin
      ...
      copyfile(pchar(folder+'A.TMP'),pchar(folder+'A.FIRST'),false);
      if terminated then exit;
      copyfile(pchar(folder+'M.TMP'),pchar(folder+'M.FIRST'),false);
    end;
