...
var
  ...
  begin
    ...
    begin
      ...
      Result := cheatenginedir + {$ifdef Darwin}'../'+{$endif}'Languages' + DirectorySeparator + LangID + DirectorySeparator + 'cheeseengine'+LCEXT;
      if FileExists(Result) then exit;

      Result := cheatenginedir + {$ifdef Darwin}'../'+{$endif}'Languages' + DirectorySeparator + LangID + DirectorySeparator + 'cheeseengine-x86_64'+LCEXT;
      if FileExists(Result) then exit;

      Result := cheatenginedir + {$ifdef Darwin}'../'+{$endif}'Languages' + DirectorySeparator + LangID + DirectorySeparator + 'cheeseengine-i386'+LCEXT;
      if FileExists(Result) then exit;
...
function altnamer(s: string): string;
begin
  s:=StringReplace(s, 'Cheat Engine','Cheese Engine',[rfReplaceAll, rfIgnoreCase]);
  ...
  s:=StringReplace(s, 'cheatengine.org','runtimemodifier.org',[rfReplaceAll]);
